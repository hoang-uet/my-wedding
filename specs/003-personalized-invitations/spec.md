...
/thiep-cuoi             → Admin dashboard (trang riêng, layout khác)
```
...
- Mỗi lần vào `/thiep-cuoi` → kiểm tra `sessionStorage` trước
...
6. **Given** admin truy cập `/thiep-cuoi` lần đầu, **When** chưa nhập PIN, **Then** hiển thị form nhập PIN. Nhập đúng PIN → vào dashboard. Nhập sai → hiển thị lỗi, cho phép thử lại.
